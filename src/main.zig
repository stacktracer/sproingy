...
const Model = struct {
    ...
    draggables: std.ArrayList( *Draggable ),
    dragger: ?*Dragger,
    axis: Axis2,

    // TODO: Would be nice to have listenables here instead
    widgetsToRepaint: std.ArrayList( *GtkWidget ),
    windowsToClose: std.ArrayList( *GtkWindow ),

    pub fn init( self: *Model, allocator: *std.mem.Allocator ) void {
        self.allocator = allocator;
        self.paintable.init( allocator );
        self.draggables = std.ArrayList( *Draggable ).init( allocator );
        self.dragger = null;
        self.axis = Axis2.create( xywh( 0, 0, 500, 500 ) );
        self.widgetsToRepaint = std.ArrayList( *GtkWidget ).init( allocator );
        self.windowsToClose = std.ArrayList( *GtkWindow ).init( allocator );
    }
...
fn onWheel( widget: *GtkWidget, ev: *GdkEventScroll, model: *Model ) callconv(.C) gboolean {
    ...
    const zoomFactor = std.math.pow( f64, zoomStepFactor, -zoomSteps );
...
fn onRender( glArea_: *GtkGLArea, glContext_: *GdkGLContext, model_: *Model ) callconv(.C) gboolean {
    return struct {
        fn run( glArea: *GtkGLArea, glContext: *GdkGLContext, model: *Model ) !gboolean {
            ...
            try model.paintable.painter.paint( viewport_PX );
            return 0;
        }
...
pub fn main( ) !void {
    ...
    bgPaintable.init( GL_COLOR_BUFFER_BIT );
    bgPaintable.rgba = [_]GLfloat { 0.0, 0.0, 0.0, 1.0 };

    var dotsPaintable = try model.paintable.addChild( DotsPaintable );
    dotsPaintable.init( &model.axis, &gpa.allocator );
